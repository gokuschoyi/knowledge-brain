...
4. content_type: Classify the chunk's primary purpose as exactly one of:
   - "definition"      — explains what something is
   - "example"         — illustrates a concept with a concrete case
   - "argument"        — makes a reasoned assertion or case
   - "data_statistics" — presents numbers, measurements, or empirical results
   - "procedure"       — describes steps or instructions
   - "reference"       — cites or points to another source
   - "narrative"       — general prose that does not fit the above
5. certainty_level (0.0–1.0): How factual and certain is the language in this chunk?
   - 0.0–0.3: highly hedged ("might", "could", "allegedly", "it is suggested")
   - 0.4–0.6: mixed certainty, some hedging
   - 0.7–0.9: mostly direct factual assertions
   - 1.0: entirely direct, unambiguous factual statements
...
- Return `grounded_citations` containing exact, contiguous `quote_text` substrings copied from the supplied chunks.
- Each citation must reference the supporting `chunk_id` and the quote must appear verbatim inside that chunk.
- Prefer fewer exact citations over broader paraphrased support.
- Only cite a chunk when the quote directly supports a claim in the answer.
- Return `answer_sections` for the major answer paragraphs or sections. Each item should contain:
  - `content`: the exact paragraph/section text as it appears in `answer_markdown`
  - `chunk_ids`: the chunk ids that most directly support that paragraph or section
- If you are unsure about a citation, omit it rather than guessing.
...
Rules:
- Write only the reader-facing answer in markdown prose.
- Do not include JSON, field names, YAML, or any structured wrapper.
- Do not output `grounded_citations`, `answer_sections`, `chunk_id`, or any schema-like labels.
- Admit uncertainty when evidence is weak.
- If contradiction warnings are present, explicitly acknowledge them instead of smoothing them over.
- For definition/explanation questions, prefer the strongest entity definitions and supporting claims.
- For precision questions, prefer exact grounded claims and chunks over generic definitions.
